...
import { insertData, fetchAllFromTable } from '../../lib/supabase';
...
const UretimGirModal: React.FC<UretimGirModalProps> = ({ isOpen, onClose, onSuccess }) => {
  // Form alanları
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const uniqueMarkalar = Array.from(new Set(musteriData.map((musteri: any) => musteri['Marka']))).filter(Boolean);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('Üretim kaydı eklenirken bir hata oluştu. Lütfen tekrar deneyin.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex items-center justify-center">
      <div className="relative bg-white rounded-lg max-w-md w-full mx-auto p-6 shadow-xl">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-semibold text-gray-900">Yeni Üretim Gir</h2>
          <button
            onClick={onClose}
        ...
        {dataLoading ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    {filteredReceteler.length === 0 ? (
                      ...
                      <ul className="divide-y divide-gray-200">
                        {filteredReceteler.map((recete, index) => (
                          ...
                          >
                            <span className="text-sm text-gray-800">{recete['Reçete Adı']}</span>
                          </li>
                        ))}
                      </ul>
                    )}
                      ...
                      <ul className="divide-y divide-gray-200">
                        {filteredMarkalar.map((item, index) => (
                          <li 
                            ...
                            className="px-4 py-3 hover:bg-indigo-50 cursor-pointer flex justify-between items-center"
                          >
                            <span className="text-sm text-gray-800">{item['Marka']}</span>
                          </li>
                        ))}
                      </ul>
                    )}
            ...
            <div>
              <label htmlFor="bulkUretimEmri" className="block text-sm font-medium text-gray-700 mb-1">
                Bulk Üretim Emri (Kg)
              </label>
              <div className="mt-1 relative rounded-md shadow-sm">
                <input
                  ...
                  onChange={(e) => handleNumericInputChange(e, setBulkUretimEmri)}
